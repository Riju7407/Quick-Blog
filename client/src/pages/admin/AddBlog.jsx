    ...
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
        ...
        <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
          ...
          {loading && (<div>
            <div className = 'absolute right-0 top-0 bottom-0 left-0 flex-items-center justufy-center bg:black/10 mt-2'>
            <div className = ' w-8 h-8 rounded-full flex-center border-2 border-t-white animate-spin'></div>

            </div>
          </div>)}
          <button disabled={loading} type='button' onClick={generateContent} className='absolute bottom-1 right-2 ml-2 text-white bg-black/70 px-4 py-1.5 rounded hover:underline cursor-pointer'>Generate with AI</button>
        </div>
